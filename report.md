...
> Capture: Registration form with filled fields, the Cypher query panel, and success message showing created user data.
...
// Password verification done in application layer using bcrypt
...
> Capture: Login form with credentials entered, successful authentication message, and user session info displayed.

![UC-2: User Login](images/uc2_user_login.png)
...
**Implementation Notes:**
- Uses `OPTIONAL MATCH` to handle users with no followers/following
- Aggregates follower and following counts in a single query
- Returns complete profile data for display
- Efficiently calculates social statistics using graph traversal

**Screenshot:** *(1 screenshot required)*

> Capture: User selection dropdown, profile display with username, name, email, bio, and follower/following statistics.

![UC-3: View Profile](images/uc3_view_profile.png)
...
// Only modifies specified fields, preserves others
...
> Capture: Edit form showing current and new values, the Cypher query, and success message with updated profile data.

![UC-4: Edit Profile](images/uc4_edit_profile.png)
...
> Capture: Follower and target user selection, Cypher query, and success message confirming the FOLLOWS relationship was created.

![UC-5: Follow Another User](images/uc5_follow_user.png)
...
> Capture: Follower and target user selection, Cypher query, and success message confirming the FOLLOWS relationship was deleted.

![UC-6: Unfollow a User](images/uc6_unfollow_user.png)
...
> Screenshot 1: User selection, Followers query, and results table listing users who follow the selected user.

![UC-7: View Followers](images/uc7_view_followers.png)

> Screenshot 2: User selection, Following query, and results table listing users the selected user follows.

![UC-7: View Following](images/uc7_view_following.png)
...
> Capture: User A and User B selection dropdowns, Cypher query, and results table/graph showing mutual connections.

![UC-8: Mutual Connections](images/uc8_mutual_connections.png)
...
**Graph Visualization:** The PyVis graph displays the center user prominently, with recommended users shown as connected nodes. Node sizes are scaled by mutual connection count, making stronger recommendations visually distinct. Dashed edges indicate potential (recommended) connections versus existing ones.

**Screenshot:** *(1 screenshot required)*

> Capture: User selection, limit slider, Cypher query, and results table showing recommended users with mutual connection counts.

![UC-9: Friend Recommendations](images/uc9_friend_recommendations.png)
...
> Capture: Search input field with a search term, Cypher query, and results table listing matching users.

![UC-10: Search Users](images/uc10_search_users.png)
...
```cypher
// UC-11: Explore Popular Users
// Returns users ranked by follower count
...
> Capture: Cypher query and results table listing the top users ranked by follower count.

![UC-11: Explore Popular Users](images/uc11_popular_users.png)

---

## Screenshot Checklist

| Use Case | Owner | Screenshots | Filename |
|----------|-------|-------------|----------|
| UC-1 | Timothy | 1 | `uc1_user_registration.png` |
| UC-2 | Timothy | 1 | `uc2_user_login.png` |
| UC-3 | Timothy | 1 | `uc3_view_profile.png` |
| UC-4 | Timothy | 1 | `uc4_edit_profile.png` |
| UC-5 | Jakob | 1 | `uc5_follow_user.png` |
| UC-6 | Jakob | 1 | `uc6_unfollow_user.png` |
| UC-7 | Jakob | 2 | `uc7_view_followers.png`, `uc7_view_following.png` |
| UC-8 | Jakob | 1 | `uc8_mutual_connections.png` |
| UC-9 | Jakob | 1 | `uc9_friend_recommendations.png` |
| UC-10 | Aditya | 1 | `uc10_search_users.png` |
| UC-11 | Aditya | 1 | `uc11_popular_users.png` |
| **Total** | | **12** | |
...
*Report generated for CS157C: NoSQL Database Systems - Final Project*